...
**S2 类中 `width > 15 µs` 的子集**（n = 12,269 / 33,608；灰色虚线标出各项 cut）：

![S2 且 width > 15 µs 的 peak 参数 2D 面板](figures/co60_590_v2_s2_w15us_2d_panels.png)

该子集的 **`n_ch` 分布**与 S2 全体、muon 类的对比：

![n_ch: S2 / long S2 / muon](figures/co60_590_v2_s2_w15us_nch.png)

| `n_ch` | S2 全体 (33,608) | S2 `width>15 µs` (12,269) | muon (15,524) |
|---|---|---|---|
| 1 | **90.52%** | **84.60%** | 0% |
| 2 | 7.45% | 11.18% | 7.85% |
| 3 | 1.87% | 3.90% | 11.80% |
...
该子集的 **sum 面积分布**（`anode_sum_area` 单峰、`dynode_sum_area` 双峰）：

![S2 (width>15 µs) 的 anode_sum_area / dynode_sum_area 分布](figures/co60_590_v2_s2_w15us_areas.png)

| 量 | 中位 | 说明 |
|---|---|---|
| `anode_sum_area` | **9,888 PE** | 单峰，log 域近似对称，10³–10⁵ PE |
| `dynode_sum_area` | **2,001 PE** | **双峰**：~3×10² PE 与 ~1.2×10⁴ PE 两簇 |
...
| run | S1 id | S2 id | **dt [µs]** | `s1_size` [PE] | `s2_size` [PE] | S2 宽度 [µs] | S1 是否落在 S2 时间跨度内 |
|---|---|---|---|---|---|---|---|
| 599 | 26889 | 26890 | **3.28** | 129.5 | 2,732.7 | 27.6 | ✅ 是 |
| 600 | 21272 | 21273 | **8.90** | 85.9 | 18,690.1 | 20.0 | ✅ 是 |
| 605 | 28818 | 28819 | **15.95** | 112.0 | 2,387.9 | 21.8 | ✅ 是 |

（`width > 10 µs` 阈值的 5 例另含 run593 `dt = 0.64 µs`、run604 `dt = 19.86 µs`；
5 例合并：min 0.64 / 中位 8.90 / max 19.86 / 均值 9.73 µs）

![S1–长S2 配对的时间间隔](figures/co60_590_v2_muon_pair_dt.png)

**配对事例的参数特征**（`width > 10 µs` 的 5 例，逐事例值见
`co60_590/muon_pair_s1_params.csv` / `muon_pair_s2_params.csv`）：

| 参数 | 配对 S1 (n=5) min/中位/max | 配对 S2 (n=5) min/中位/max | 参照 |
|---|---|---|---|
| `n_ch` | **1 / 1 / 1** | **1 / 1 / 1** | muon S1 n_ch = 7 |
| `anode_sum_area` [PE] | **86 / 112 / 143** | 2,388 / 8,829 / 23,621 | S1 类中位 107；**muon S1 段中位 3,649**；long-S2 类中位 10,086 |
| `dynode_sum_area` [PE] | 0 / 0 / 144 | 0 / 341 / 29,674 | 4/5 例 dynode 完全无信号 |
| `height` [ADC] | 1,151 / 1,972 / 2,599 | 2,346 / 9,944 / 12,430 | S1 类中位 1,687；S2 类中位 4,633；cut = 15,000 |
| `width` [ns] | 276 / 436 / 752 | 10,752 / 19,952 / 27,636 | — |
| `rise_time` [ns] | 12 / 16 / 20 | 12 / 384 / 2,340 | — |
| `width_90area` [ns] | 91 / 108 / 373 | 5,429 / 13,828 / 23,379 | — |

![配对事例的参数位置（相对各类总体）](figures/co60_590_v2_muon_pair_params.png)

**五个配对事例的波形**（每对一张图，两 peak 置于同一绝对时间轴，t=0 取 S1 脉冲起点）：

![pair run593 S1 id=38507 + S2 id=38508](figures/co60_590_v2_muon_pair_waveforms_run593.png)
...
![pair run604 S1 id=17266 + S2 id=17267](figures/co60_590_v2_muon_pair_waveforms_run604.png)

![pair run605 S1 id=28818 + S2 id=28819](figures/co60_590_v2_muon_pair_waveforms_run605.png)

| run | dt [µs] | S1 面积 [PE] | S1 `dynode_sum_area` [PE] | S2 面积 [PE] | S2 `height` [ADC] | 波形判读 |
|---|---|---|---|---|---|---|
| 593 | **0.64** | 142.8 | **144.2（有 dynode 信号）** | 8,828.7 | 4,486 | ⚠️ dt 极短且两侧**都有 dynode 信号**，S1 的 dynode 摆动与 S2 的阳极上升沿几乎同时——更像是**同一次物理事件被聚类切成两个 peak** |
| 599 | 3.28 | 129.5 | 0 | 2,732.7 | 2,346 | S1 无 dynode；S2 是自带上升沿的 27.6 µs 噪声平台 |
| 600 | 8.90 | 85.9 | 0 | 18,690.1 | 12,430 | S2 是**形态完整的大脉冲**，S1 仅是 1.2 kADC 小尖峰 |
...
> 两侧均有 dynode、波形紧邻），更像是聚类把一段连续波形切成了两个 peak。
...
> 配对到的 S2 则完全落在 long-S2 类总体内（面积中位 8,829 vs 总体 10,086 PE），
> 无特殊性。

> ⚠️ **3/3（5 例中 4/5）配对到的 S1 在时间上落在该长 S2 自身的时间跨度内**
> （`dt < S2 宽度`）——即这两个 peak 在时间上是**重叠/紧邻**的，不是「先 S1 后 S2」的
> 物理先后关系，更像是同一段波形被聚类切分成了两个 peak。

**结论：配对率与随机符合一致，不存在物理上的 S1–长S2 关联。**

- S1 事例率 ≈ **15 Hz**（run 590：25,952 个 / 2,427 s，间隔中位 **65.6 ms**）
- 长 S2 事例率 ≈ **0.45 Hz**（run 590：753 个 / 2,427 s，间隔中位 **2,219 ms**）
- 25 µs 窗内随机命中的期望概率 ≈ 25 µs × 15 Hz ≈ **0.037%**
  → 17,337 × 0.037% ≈ **6.4 例预期**，实测 5 例 ✓
- 12,269 个长 S2 到「最近的前一个 S1」的时间差：**中位 65,385 µs（65.4 ms）**，
  q1 = 1,110 µs，q99 = 447,983 µs，**仅 0.0245% 落在 25 µs 内**

**因此本数据集中的长 S2 事例不是「S1 + 延迟 S2」型 muon 事例**：两者的时间间隔是
