...
`muon_s1_rise_ns = (S1 peak − muon_s1_start) × 4 ns`，S1 peak = anode_sum 的 argmin。

Co60 590+ v2（n=15,524）：中位 **20 ns**（q25 16 / q75 24 / q95 28），
但存在 100-3000 ns 的慢上升尾部（q99=372 ns）——**高面积组（≥2.5e3 PE）特有**，
低面积组（<2.5e3）中位 16 ns 且几乎无尾部。图：`co60_590_v2_muon_s1_rise.png`。

### muon_s2 上升沿（**已放弃**）

定义为 `(S2 peak − muon_s1_end) × 4 ns`（S2 peak = S2 窗口内 argmin）时
**70% 事例结果为 0**，分布严重退化：

- `muon_s1_end`（3B 切分点）落在 S1 下降段**中部**（峰值后仅 ~100 ns），
  故 S2 窗口起点即窗口内最负点 → rise ≡ 0
- 与 3B 在饱和削顶波形上的伪影同源（见 §"注意"）
- **结论：该定义无物理意义，放弃**。如需 S2 上升沿，应先修正 S1/S2 切分点
  （改用下降段局部极小 / 平台到达判据）。
